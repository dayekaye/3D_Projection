...
def computefaces(corner1, corner2, vanishingpt, imwidth, imheight, focallen):
    ...
    # print "backright", backleft
    # print "backleft", backright
    # print "backbottom", backbottom
    # print "backtop", backtop
    # print "vanishing point : ",vanishingpt
    ...
    #basic arrays were going to play with

    #equals backtop - vanishingpoint[1]
    #f/a = (f+d)/h
    #d=(f*h/a)-f

    a = backtop - vanishingpt[1]
    d = (focallen*h/a)-focallen

    
    back3d = np.array([[imwidth, imheight, d], #top right
                        [0, imheight, d], #top left
                        [0, 0, d],   #bottom right
                        [imwidth, 0, d]]) #bottom left

    right3d = np.array([[getX(imheight, [backright, backbottom], vanishingpt), getY(imwidth, [backright, backbottom], vanishingpt), 0], #front top
                        [imwidth, imheight, d], #back bottom
                        [imwidth, 0, d],    #back top
                        [imwidth, getY(imwidth, [backright, backtop], vanishingpt), 0]])   #front bottom

    bottom3d = np.array([[imwidth, 0, d],  #back left
                        [0, 0, d],    #back right
                        [getX(imheight, [backleft, backbottom], vanishingpt), getY(0, [backleft, backtop], vanishingpt), 0],   #front left
                        [imwidth, getY(imwidth, [backright, backtop], vanishingpt), 0]])                                       #frint right

    left3d = np.array([[getX(imheight, [backleft, backbottom], vanishingpt), getY(0, [backleft, backbottom], vanishingpt), 0],    #front top
                        [0, imheight, d],  #back top
                        [0, 0, d],         #back bottom
                        [getX(imheight, [backleft, backbottom], vanishingpt), getY(0, [backleft, backtop], vanishingpt), 0]])   #front bottom

    top3d = np.array([[imwidth, imheight, d],   #back 
                        [0, imheight, d],                                                #back 
                        [getX(imheight, [backleft, backbottom], vanishingpt), getY(0, [backleft, backbottom], vanishingpt), 0],  #front left                                                 #front left
                        [getX(imheight, [backright, backbottom], vanishingpt), getY(imwidth, [backright, backbottom], vanishingpt), 0]])                                      #front right

    #print back3d, right3d, top3d, left3d, bottom3d, back2d, right2d, top2d, left2d, bottom2d
...
def computehomography(facefrom, faceto):
    ...
    #see page 81 of slides for building A
    faceIndex = 0
    for i in range(0,7,2):
        #print "i: ", i
        A[i][0] = facefrom[faceIndex][0]
        A[i][1] = facefrom[faceIndex][1]
        A[i][2] = 1
        A[i][6] = -(faceto[faceIndex][0]*facefrom[faceIndex][0])
        A[i][7] = -(faceto[faceIndex][0]*facefrom[faceIndex][1])
        A[i][8] = -(faceto[faceIndex][0])

        A[i+1][3] = facefrom[faceIndex][0]
        A[i+1][4] = facefrom[faceIndex][1]
        A[i+1][5] = 1
        A[i+1][6] = -(faceto[faceIndex][1]*facefrom[faceIndex][0])
        A[i+1][7] = -(faceto[faceIndex][1]*facefrom[faceIndex][1])
        A[i+1][8] = -(faceto[faceIndex][1])
        faceIndex += 1
    print "return homagraphy : ", np.linalg.svd(np.dot(A.T, A))[2][-1, :].reshape((3, 3))
...
def homographywarp(source, homography, imwidth, imheight):
    ...
    for y in xrange(imheight):
        for x in xrange(imwidth):
            #print source[x][y]
            #
            soureCoords = np.linalg.solve(homography, [x,y,1])

            soureCoords /= soureCoords[2]
            #print "soureCoords: ", soureCoords
            if int(soureCoords[0]) >= 0 and int(soureCoords[0]) < source.shape[1] and int(soureCoords[1]) >= 0 and int(soureCoords[1]) < source.shape[0]:
                newimage[y,x] = source[int(soureCoords[1]), int(soureCoords[0])]
...
class Navigate():
    ...
    def activate(self, corner1, corner2, vanishingpt):
        ...
        # debugging
        Image.fromarray(backimg).save("debug_back1.png")
        Image.fromarray(rightimg).save("debug_right1.png")
        Image.fromarray(topimg).save("debug_top1.png")
        Image.fromarray(leftimg).save("debug_left1.png")
        Image.fromarray(bottomimg).save("debug_bottom1.png")
